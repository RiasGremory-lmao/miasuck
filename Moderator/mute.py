...
tunhan = 948908350060503050
mutechannel = 948793932941234218
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, member: discord.Member, time='___',*, reason: str = '___'):
        ...
        if time == '___':
            ...
            embed = discord.Embed(title='Toà án Tearmoon tối cao',
                                  description='Phán xét',
                                  colour=discord.Colour.blue())
            embed.set_thumbnail(url=member.avatar_url)
            embed.add_field(name='Thẩm phán',
                            value=ctx.author.mention,
                            inline=False)
            embed.add_field(name='Bị cáo', value=member.mention, inline=False)
    ...
    async def mute_error(self, ctx, error):
        if isinstance(error, CheckFailure):
            await ctx.send("Mia chỉ nghe lệnh từ cấp trên!")
